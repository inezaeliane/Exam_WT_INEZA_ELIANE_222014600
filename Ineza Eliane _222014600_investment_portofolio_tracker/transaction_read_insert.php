...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        header("Location:transaction_read_insert.php");
...
$sql_select = "SELECT * FROM transaction";
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    // Add search condition to SQL query
    $sql_select .= " WHERE Investment_id LIKE '%$search%' OR Transaction_type LIKE '%$search%' OR 	Amount LIKE '%$search%' 
    OR Transaction_price LIKE '%$search%' OR Transaction_date LIKE '%$search%'OR 	Fees LIKE '%$search%' OR Taxes  LIKE '%$search%'";
}
...
<div class="sidebar">
    ...
    <div class="container">
        ...
        <a href="transaction.php" class="btn btn-success">Add New</a>
    </div><br>
    ...
    <table id="dataTable" class="table table-hover text-center">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>" . $row["Transaction_price"] . "</td>";
                echo "<td>" . $row["Transaction_date"] . "</td>";
                echo "<td>" . $row["Fees"] . "</td>";
                echo "<td>" . $row["Taxes"] . "</td>";
                echo "<td>";
                echo "<a href='transactionupdate.php?updateTransaction_id =" . $row['Transaction_id'] . "'>Update</a> | ";
                echo "<a href='transactiondelete.php?Transaction_id=" . $row['Transaction_id'] . "'>Delete</a>";
                 echo "</td>";
                echo "</tr>";
            }
        } else {
