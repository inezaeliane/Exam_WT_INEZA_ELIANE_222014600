...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO real_estate_investments (Property_name, Location, Purchase_price, Market_value, Rental_income, Expenses) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    // Bind parameters
    $stmt->bind_param("ssssss", $pname, $location, $pprice, $mvalue, $rincome, $expense);
    ...
    if ($stmt->execute()) {
        header("Location: real_estate_investment.php");
...
$sql_select = "SELECT * FROM real_estate_investments";
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    // Add search condition to SQL query
    $sql_select .= " WHERE Property_name LIKE '%$search%' OR Location LIKE '%$search%' OR Purchase_price LIKE '%$search%' 
    OR Market_value LIKE '%$search%' OR Rental_income LIKE '%$search%' OR 	Expenses LIKE '%$search%'";
}
...
<div class="sidebar">
    ...
    <div class="container">
        ...
        <a href="real_estate_investment.php" class="btn btn-success">Add New</a>
    </div><br>
    ...
    <table id="dataTable" class="table table-hover text-center">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>" . $row["Rental_income"] . "</td>";
                echo "<td>" . $row["Expenses"] . "</td>";
                echo "<td>";
                echo "<a href='realupdate.php?updateProperty_id=" . $row['Property_id'] . "'>Update</a> | ";
                echo "<a href='realdelete.php?Property_id=" . $row['Property_id'] . "'>Delete</a>";
                 echo "</td>";
                echo "</tr>";
            }
        } else {
